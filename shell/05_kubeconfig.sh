...
for instance in kn1 kn2 kn3; do
  scp -i $SSHKEY \
    ${instance}.kubeconfig kube-proxy.kubeconfig ${SSHUSR}@${instance}:~/
done
Copy the appropriate kube-controller-manager and kube-scheduler kubeconfig files to each controller instance:

for instance in kc1 kc2 kc3; do
  scp -i $SSHKEY \
    admin.kubeconfig kube-controller-manager.kubeconfig kube-scheduler.kubeconfig ${SSHUSR}@${instance}:~/
done
